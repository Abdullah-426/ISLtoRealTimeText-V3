...
def _resample_to_T(x, out_T: int):
    """Linear time-resampling to exactly out_T frames."""
    T, D = x.shape
    if T == out_T:
        return x
...
# -------------------- Augmentation wrapper (ALWAYS returns target_len) -----------------
...
def build_seq_augment_fn(
    ...
    def tf_aug(x, y):
        def _aug(a):
            # Ensure NumPy array (fixes EagerTensor .copy issue)
            a = np.asarray(a, dtype=np.float32)

            # 1) time-domain transforms → variable length
            ...
            # 4) force back to fixed length
            a = _resample_to_T(a, target_len)
            return a.astype(np.float32)

        # Use numpy_function so inputs arrive as NumPy arrays
        x_out = tf.numpy_function(_aug, [x], tf.float32)
        x_out.set_shape([target_len, FEAT_DIM])  # static shape for TF
        return x_out, y
    return tf_aug
...
def build_lstm_model(
    ...
    y = K.layers.Bidirectional(
        K.layers.LSTM(lstm_w2, return_sequences=True,
                      kernel_regularizer=reg, recurrent_regularizer=reg)
    )(x)
    ...
    model = K.Model(inp, out, name="ISL_BiLSTM_Attn_v4")
...
def build_tcn_model(
    ...
    model = K.Model(inp, out, name="ISL_TCN_v4")
    loss_fn = make_sparse_ce_with_smoothing(num_classes, label_smoothing)
    model.compile(optimizer=K.optimizers.Adam(learning_rate=lr, clipnorm=1.0),
                  loss=loss_fn, metrics=[])
    return model
...
def main():
    ap = argparse.ArgumentParser(
        description="Train ISL sequence model (LSTM/TCN) with robust temporal augmentations (v4).")
    ...
    ap.add_argument("--save_dir", type=str, default="models/isl_seq_model_v4")
    ...
    # Regularization & Augmentation controls (v4 tuned defaults)
    ...
    # Temporal warping/crop to undo collection pacing
    ap.add_argument("--time_warp", action="store_true",
                    help="Enable random speed jitter")
    ...
    ap.add_argument("--temporal_crop", action="store_true",
                    help="Enable random crop & resize in time")
    ...
    # Train subset controls (reduce overfitting)
    ...
    ds_te = make_dataset(
        ...
        augment=False, cache=cache_te, deterministic=True,
        time_shift=0, face_scale=1.0, face_dropout=0.0,
        xy_scale=0.0, xy_shift=0.0, z_noise=0.0,
        temporal_cutout=0, shuffle_buf=1024,
        hand_dropout=0.0, landmark_dropout=0.0,
