...
def process_one_clip(clip_dir: str,
    ...
    Returns dict with status and source_root for attribution.
    """
    clip_dir = Path(clip_dir)
    dest_class_dir = Path(dest_class_dir)
    info = {"ok": False, "written": False, "reason": None,
            "new_index": None, "source_root": source_root}
...
def collect_all_source_clips_for_label(label: str, sources_roots):
    """
    Returns ordered list of (source_index, clip_path) for this label.
    """
    ...
    return [(si, p) for (si, _idx, p) in flat]
...
def merge_and_qc_main():
    ...
    # Helper to make a stable printed name for sources
    source_names = [str(s) for s in sources]

    for lab in LABELS:
        cdir = label_to_dir[lab]
        clips_src_pairs = collect_all_source_clips_for_label(lab, sources)
        if not clips_src_pairs:
            summary["classes"][lab] = {
                "source_clips": 0, "written": 0, "dropped": 0, "dropped_by_source": {}
            }
            continue
        ...
        written = len(created)
        dropped = len(clips_src_pairs) - written
        grand_written += written
        grand_dropped += dropped

        # ----- NEW: Aggregate drops by source -----
        drops_by_source = {name: 0 for name in source_names}
        for r in results:
            # If not written => this clip was dropped
            if not r.get("written", False):
                src = r.get("source_root", "UNKNOWN")
                drops_by_source[src] = drops_by_source.get(src, 0) + 1

        # Keep only non-zero entries for cleaner output
        drops_by_source_nz = {k: v for k,
                              v in drops_by_source.items() if v > 0}

        summary["classes"][lab] = {
            "source_clips": len(clips_src_pairs),
            "written": written,
            "dropped": dropped,
            "dropped_by_source": drops_by_source_nz
        }

        # Print per-class line as before
        print(
            f"[CLASS] {lab:20s}  source={len(clips_src_pairs):4d}  kept={written:4d}  dropped={dropped:4d}")

        # ----- NEW: Print drop origins if any -----
        if drops_by_source_nz:
            parts = [f"{k}={v}" for k, v in drops_by_source_nz.items()]
            print(f"[DROPS] {lab:20s}  " + ", ".join(parts))
